...
namespace Material.Blazor.Internal
{
    ...
    public abstract class InputComponent<T> : ComponentFoundation
    {
        ...
        private protected T ComponentValue
        {
            ...
            set
            {
                LogTrace($"ComponentValue setter entered: _componentValue is '{_cachedValue?.ToString() ?? "null"}' and new value is'{value?.ToString() ?? "null"}'");

                if (!EqualityComparer<T>.Default.Equals(value, _componentValue))
                {
                    LogTrace($"ComponentValue setter changed _componentValue");
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (EditContext != null && IgnoreFormField)
            {
                LogWarning($"{GetType()} is in a form but has EditContext features disabled because it is considered a valid Material.Blazor form field type");
            }
        }
        ...
        public override async Task SetParametersAsync(ParameterView parameters)
        {
            ...
            if (!_hasSetInitialParameters)
            {
                ...
                LogTrace($"SetParametersAsync setting ComponentValue value to '{Value?.ToString() ?? "null"}'");
        ...
        {
            LogTrace($"OnParametersSet setter entered: _cachedValue is '{_cachedValue?.ToString() ?? "null"}' and Value is'{Value?.ToString() ?? "null"}'");

            if (!EqualityComparer<T>.Default.Equals(_cachedValue, Value))
            {
                _cachedValue = Value;

                LogTrace($"OnParametersSet changed _cachedValue value");

                if (!EqualityComparer<T>.Default.Equals(_componentValue, Value))
                {
                    LogTrace("OnParametersSet update _componentValue value from '" + _componentValue?.ToString() ?? "null" + "'");
