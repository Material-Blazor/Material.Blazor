...
internal class TooltipService : IMBTooltipService
{
    ...
    public void AddTooltip(long id, RenderFragment content)
    {
        ...
        {
            throw new InvalidOperationException($"Material.Blazor.MD2: you attempted to add a tooltip from a {Utilities.GetTypeName(typeof(IMBTooltipService))} but have not placed an MBAnchor component at the top of either App.razor or MainLayout.razor");
        }
    ...
    public void RemoveTooltip(long id)
    {
        if (OnRemove is null)
        {
            throw new InvalidOperationException($"Material.Blazor.MD2: you attempted to remove a tooltip from a {Utilities.GetTypeName(typeof(IMBTooltipService))} but have not placed an MBAnchor component at the top of either App.razor or MainLayout.razor");
        }
