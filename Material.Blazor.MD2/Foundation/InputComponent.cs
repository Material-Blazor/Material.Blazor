...
public abstract class InputComponent<T> : ComponentFoundation
{
    ...
    /// <summary>
    /// Gets or sets the value of the component. To be used by Material.Blazor.MD2 components for binding to
        ...
        set
        {
            ...
            if (!EqualityComparer<T>.Default.Equals(value, _componentValue))
            {
                ...
                if (EditContext != null && !IgnoreFormField)
                {
                    ...
                    {
                        throw new Exception("Material.Blazor.MD2: ValueExpression must be defined for a field contained in an EditForm");
                    }
                    else
                    {
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (EditContext != null && IgnoreFormField)
        {
            LoggingService.LogWarning($"{GetType()} is in a form but has EditContext features disabled because it is considered a valid Material.Blazor.MD2 form field type");
        }
    }
    ...
    /// <summary>
    /// Material.Blazor.MD2 components descending from MdcInputComponentBase _*must not*_ override ShouldRender().
    /// </summary>
